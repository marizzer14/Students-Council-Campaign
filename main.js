...
(function() {
    ...
    if (window.innerWidth <= 768) {
        ...
        console.log('📱 Mobile mode activated on load');
    }
...
function init() {
    ...
    // Clear any existing back button first
    const existingBackBtn = document.getElementById('voterBackBtn');
    if (existingBackBtn) {
    ...
    backBtn.id = 'voterBackBtn';
    backBtn.innerHTML = '← Back to Role Selection';
    ...
    // Insert at the top of registration content
    registrationContent.insertBefore(backBtn, registrationContent.firstChild);
});
...
function init() {
    ...
    // Show admin code section
    document.getElementById('adminCodeSection').style.display = 'block';
    this.style.display = 'none';
    
    // Remove existing back button if any
    const existingBackBtn = document.getElementById('adminBackBtn');
    if (existingBackBtn) {
        existingBackBtn.remove();
    }
    ...
    // Insert at the top of admin code section
    adminCodeSection.insertBefore(backBtn, adminCodeSection.firstChild);
});
    document.getElementById('verifyAdminBtn').addEventListener('click', function() {
        const code = document.getElementById('adminCode').value;
        if (code === appState.adminCode) {
    ...
    document.getElementById('continueAsVoter').addEventListener('click', function() {
        ...
        // Add back button to registration modal if not already there
        if (!document.getElementById('voterBackBtn')) {
            const registrationContent = document.querySelector('.registration-content');
            const backBtn = document.createElement('button');
            backBtn.id = 'voterBackBtn';
            backBtn.innerHTML = '← Back to Role Selection';
...
// FOR TESTING ONLY - creates a button to toggle mobile view
window.addEventListener('load', function() {
    const toggleBtn = document.createElement('button');
    toggleBtn.textContent = '📱 Toggle Mobile View';
    ...
    toggleBtn.onclick = function() {
        ...
        console.log('Mobile mode:', document.body.classList.contains('mobile-device'));
    };
